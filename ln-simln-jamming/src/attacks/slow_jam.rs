    ...
    J: ChannelJammer + Send + Sync,
{
    ...
    async fn fast_jam_channel(
        ...
        loop {
            ...
            // TODO: Instead of this, check if a payment immediately fails. That likely means we
            // do not have enough reputation to continue jamming the channel.
...
#[async_trait]
impl<C, R, M, J> JammingAttack for SlowJam<C, R, M, J>
where
    C: Clock + InstantClock,
    R: ReputationMonitor + Send + Sync,
    M: PeacetimeRevenueMonitor + Send + Sync,
    J: ChannelJammer + Send + Sync,
{
    async fn setup_for_attack(
        ...
        // TODO: account for fee paid to build reputation.
    ...
    async fn run_custom_actions(
        ...
        // We build reputation for the target channel in [`setup_for_attack`] so we assume we
        // already have reputation to start the attack. With sufficient reputation we then:
        ...
        self.fast_jam_channel(attacker_nodes).await?;
    ...
    async fn simulation_completed(
        ...
        // Instead of this check, return true if reputation is close to 0?
        if !self.sufficient_reputation().await? {
