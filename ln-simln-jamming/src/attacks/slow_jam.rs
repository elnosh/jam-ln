    ...
    J: GeneralChannelJammer + Send + Sync,
{
    ...
    // jamming it. This is used similar to the shutdown conditions for simulation in general where
    // it will check if the attacker has sufficient reputation for "a reasonable htlc amount" by
    // using the risk margin.
