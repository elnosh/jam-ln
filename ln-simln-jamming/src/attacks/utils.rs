...
pub async fn build_reputation<C: Clock + InstantClock, R: ReputationMonitor>(
    ...
    let shutdown = triggered::trigger();
    let payment_result = attacker.track_payment(&payment_hash, shutdown.1).await?;

    match payment_result.payment_outcome {
        PaymentOutcome::Success => {}
        _ => return Err("payment failed to build reputation failed".into()),
    }
