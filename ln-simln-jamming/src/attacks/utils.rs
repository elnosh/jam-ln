...
fn fee_to_build_reputation(
    ...
    for route in htlc_routes {
        for path in &route.paths {
                ...
                .position(|hop| hop.pubkey == target_hop_pubkey)
            {
                ...
                // could case the calculated fee here to not be enough. Hence, we default to
                // a reasonably higher value of 200 which overpays a little but should be enough
                // to cover the random offsets.
                let cltv_expiry: u32 = path.hops[target_idx..].iter().len() as u32 * 200;

                let target_hop = &path.hops[target_idx];
                let current_htlc_risk =
                    forward_params.htlc_opportunity_cost(target_hop.fee_msat, cltv_expiry);
    ...
    let fee_to_bump_reputation = threshold + total_htlc_risk + fee_buffer;
...
mod tests {
    ...
    use ln_resource_mgr::{
        ...
        AccountableSignal, ReputationParams,
    };
    ...
    use crate::{
        ...
        attacks::utils::{build_custom_route, build_reputation, BuildReputationParams},
        records_from_signal,
    ...
    /// to build reputation.
    // Took mostly from `setup_three_hop_network_edges` but wit the extra node and channel
    ...
    fn test_calculated_fee_to_build_reputation() {
        ...
        let htlc_routes: Vec<Route> = htlc_amounts
            .iter()
            .map(|amount| {
        ...
        // - Fee buffer -> 1000
        let expected_fee = 300_000 + expected_htlc_risk + 1000;
