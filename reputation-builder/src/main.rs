        ...
        match cli.target_alias {
            Some(target) => {
                ...
                let target_to_attacker = match sim_network.iter().find(|&channel| {
                    ...
                    {
                        return true;
                    } else if channel.node_1.pubkey == attacker_pubkey
                    ...
                    None => return Err(format!("no channel between target and attacker").into()),
                };
            ...
            None => return Err(format!("target not provided").into()),
        }
    } else {
